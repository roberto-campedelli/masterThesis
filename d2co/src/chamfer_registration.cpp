...
double ChamferRegistration::avgDistance( int idx )
{
  int n_pts = 0;
  double avg_dist = 0;
...
void ChamferRegistration::updateOptimizer( int idx )
{
...
double ICPChamferRegistration::avgDistance ( int idx )
{
  int n_pts = 0;
  double avg_dist = 0;
...
void ICPChamferRegistration::updateOptimizer ( int idx )
{
  selected_idx_ = idx;
...
double DirectionalChamferRegistration::avgDistance( int idx )
{
...
void DirectionalChamferRegistration::updateOptimizer( int idx )
{
  const std::vector<Point3f> &model_pts = ( idx < 0 )?model_ptr_->getPoints():model_ptr_->getPrecomputedPoints(idx);
...
double HybridDirectionalChamferRegistration::avgDistance ( int idx )
{
  int n_pts = 0;
  double avg_dist = 0;
...
void HybridDirectionalChamferRegistration::updateOptimizer ( int idx )
{
  selected_idx_ = idx;
...
double BidirectionalChamferRegistration ::avgDistance( int idx )
{
  std::cerr<<"IMPLEMENT ME!!"<<std::endl;
  return -1;
}
...
void BidirectionalChamferRegistration ::updateOptimizer( int idx )
{
  const std::vector<Point3f> &model_pts = ( idx < 0 )?model_ptr_->getPoints():model_ptr_->getPrecomputedPoints(idx);
  const std::vector<Point3f> &model_dpts = ( idx < 0 )?model_ptr_->getDPoints():model_ptr_->getPrecomputedDPoints(idx);
...
double ICPDirectionalChamferRegistration::avgDistance ( int idx )
{
  int n_pts = 0;
  double avg_dist = 0;
...
void ICPDirectionalChamferRegistration::updateOptimizer ( int idx )
{
  selected_idx_ = idx;
  model_pts_ = ( idx < 0 )?model_ptr_->getPoints():model_ptr_->getPrecomputedPoints(idx);
}
...
double MultiViewsDirectionalChamferRegistration::avgDistance( int idx )
{
  // TODO
  return -1;
//   int n_pts = 0;
...
void MultiViewsDirectionalChamferRegistration::updateOptimizer( int idx )
{
  optimizer_ptr_ = std::shared_ptr< Optimizer > ( new Optimizer () );

  for( int i = 0; i < static_cast<int>(model_ptrs_.size()); i++ )
  {
    const std::vector<Point3f> &model_pts = ( idx < 0 ) ? model_ptrs_[i]->getPoints()
                                                        : model_ptrs_[i]->getPrecomputedPoints(idx);
    const std::vector<Point3f> &model_dpts = ( idx < 0 ) ? model_ptrs_[i]->getDPoints()
                                                         : model_ptrs_[i]->getPrecomputedDPoints(idx);
