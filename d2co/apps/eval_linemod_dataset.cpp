...
int main(int argc, char **argv)
{
  ...
  Projection2DMetric proj_2d_metric( obj_model_ptr );
  Pose6DMetric pose_6d_metric(obj_model_ptr, obj_diameter_cm );
...
//      }
      ...
      if( !done_with_55 )
      {
        if( max_rt_metric.performTest(match_r_quat, match_t_vec, options.symmetric_object ) )
        {
          dcm_55[i_m]++;
          done_with_55 = true;
        }
      ...
      if( !done_with_2dp )
      {
        if( proj_2d_metric.performTest( match_r_quat, match_t_vec, options.symmetric_object ) )
        {
          dcm_2dp[i_m]++;
          done_with_2dp = true;
...
//          }
      ...
      if( !done_with_p6d )
      {
        if( pose_6d_metric.performTest( match_r_quat, match_t_vec, options.symmetric_object ) )
        {
          dcm_p6d[i_m]++;
          done_with_p6d = true;
        }
