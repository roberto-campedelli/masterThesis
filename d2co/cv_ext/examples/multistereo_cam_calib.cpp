...
{
  putText(img, text, pos, CV_FONT_HERSHEY_PLAIN, font_size, Scalar(0,0,0), 2, 8);
  putText(img, text, pos, CV_FONT_HERSHEY_PLAIN, font_size, color, 1, 8);
}
...
int main(int argc, char **argv)
{
  ...
  {
    grid_size.width = ceil(sqrt(num_cameras*16.0/9.0));
    grid_size.height = round(num_cameras/grid_size.width);
  }
  ...
  while ( !time_to_exit )
  {
    if( update_img )
    {
      calib.getCornersImageTuple(tuple_idx, corners_img, scale_factor);
      stringstream info_01, info_11;
      info_01<<"Tuple "<<tuple_idx<<"/"<<calib.numImageTuples()<<"; epipolar RMSE : "
             <<calib.getEpipolarError(tuple_idx);

      if( !calib.isImageTupleActive(tuple_idx) )
      {
        info_01<<" - INACTIVE";
        corners_img_grid *= 0.1;
      }

      info_11<<"# used for calibartion : "<<calib.numActiveImageTuples()
      ...
      calib.getCornersDistribution( 30, corner_dist, scale_factor);
      for( int k = 0; k < num_cameras; k++ )
      {
    ...
    switch( cv_ext::waitKeyboard() )
    {
      case 'n':
      case 'N':
        if( ++tuple_idx >= calib.numImageTuples() )
          tuple_idx = 0;
        update_img = true;
        break;
      ...
      case 'P':
        if( --tuple_idx < 0 )
          tuple_idx = calib.numImageTuples() - 1;
        update_img = true;
        break;
      case ' ':
        calib.setImageTupleActive( tuple_idx, !calib.isImageTupleActive(tuple_idx) );
        update_img = true;
        break;
      case 'c':
      case 'C':
