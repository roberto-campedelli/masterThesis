...
int main(int argc, char **argv)
{
  ...
  while ( !time_to_exit )
  {
    if( update_img )
    {
      calib.getCornersImage(img_idx,corners_img, scale_factor);      
      
      stringstream info_01, info_11;
      info_01<<"Image "<<img_idx<<"/"<<calib.numImages()<<"; reprojection error : "
             <<calib.getReprojectionError(img_idx);
      
      if( !calib.isImageActive(img_idx) )
      {
        info_01<<" - INACTIVE";
        corners_img *= 0.1;
      }
      
      info_11<<"# used for calibartion : "<<calib.numActiveImages()
    ...
    switch( cv_ext::waitKeyboard() )
    {
      ...
      case 'P':
        if( --img_idx < 0 ) 
          img_idx = calib.numImages() - 1;
        update_img = true;
        break;
      case ' ':
        calib.setImageActive( img_idx, !calib.isImageActive(img_idx) );
        update_img = true;
        break;
      case 'c':
      case 'C':
  ...
  if ( show_undistorted && rep_error != std::numeric_limits<double>::infinity() )
  {
    ...
    while ( !time_to_exit )
    {
      if(update_img)
      {
        ...
        calib.getUndistortedImage(img_idx,und_img,scale_factor);
