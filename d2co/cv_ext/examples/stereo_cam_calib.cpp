...
int main(int argc, char **argv)
{
  ...
  while ( !time_to_exit )
  {
    if( update_img )
    {
      calib.getCornersImagePair(pair_idx, corners_img, scale_factor);
      stringstream info_01, info_11;
      info_01<<"Pair "<<pair_idx<<"/"<<calib.numImagePairs()<<"; epipolar error : "
             <<calib.getEpipolarError(pair_idx);
      
      if( !calib.isImagePairActive(pair_idx) )
      {
        info_01<<" - INACTIVE";
        corners_img_pair *= 0.1;
      }
      
      info_11<<"# used for calibartion : "<<calib.numActiveImagePairs()
    ...
    switch( cv_ext::waitKeyboard() )
    {
      case 'n':
      case 'N':
        if( ++pair_idx >= calib.numImagePairs() )
          pair_idx = 0;
        update_img = true;
        break;
      ...
      case 'P':
        if( --pair_idx < 0 ) 
          pair_idx = calib.numImagePairs() - 1;
        update_img = true;
        break;
      case ' ':
        calib.setImagePairActive( pair_idx, !calib.isImagePairActive(pair_idx) );
        update_img = true;
        break;
      case 'c':
      case 'C':
  ...
  {    
    ...
    while ( !time_to_exit )
    {
      ...
      {
        Mat und_img;
        calib.rectifyImagePair(pair_idx,rect_img,scale_factor);
